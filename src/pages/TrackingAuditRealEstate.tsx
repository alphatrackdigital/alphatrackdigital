...
const ROLE_OPTIONS = [
  ...
  { value: "marketing_lead", label: "Marketing / Sales" },
  { value: "growth_performance", label: "Growth / Performance" },
  { value: "operations_commercial", label: "Sales / Operations" },
...
const CONVERSION_OPTIONS = [
  { value: "lead_form", label: "Property enquiry form" },
  { value: "booked_call_appointment", label: "Booked viewing / appointment" },
  { value: "whatsapp_message", label: "WhatsApp / message" },
  { value: "other", label: "Other property lead action" },
] as const;

const PROBLEM_OPTIONS = [
  { value: "unclear_campaign_performance", label: "I can’t tell which ads bring qualified property enquiries" },
  { value: "conflicting_numbers", label: "Ads and analytics don’t agree" },
  { value: "missing_conversion_tracking", label: "Some enquiries or viewings aren’t tracked" },
  { value: "leads_without_attribution", label: "Enquiries reach sales without campaign source" },
...
const MEASUREMENT_JOURNEY = [
  ...
  { icon: Globe2, title: "Property / listing page" },
  { icon: Send, title: "Enquiry / viewing" },
  { icon: Gauge, title: "Sales / CRM" },
] as const;

const JOURNEY_BREAKS = [
  { label: "Campaign source lost", position: "25%" },
  { label: "Viewing event missed", position: "50%" },
  { label: "Source not passed through", position: "75%" },
] as const;

const REPORTING_SNAPSHOT = [
  { label: "Ad platform", value: "22 leads", detail: "Reports 22 leads." },
  { label: "Analytics", value: "14 enquiries", detail: "Shows 14 enquiries." },
  { label: "Sales / CRM", value: "17 enquiries", detail: "Receives 17 enquiries." },
] as const;
...
const SCORECARD_PREVIEW = [
  {
    ...
    detail: "Booked-viewing event is not being recorded consistently.",
    tone: "warning",
  },
  {
    label: "Attribution",
    status: "Partial",
    detail: "Campaign source reaches the property page but is lost before the sales handoff.",
    tone: "partial",
  },
  {
    label: "Lead Visibility",
    status: "Needs attention",
    detail: "Some enquiries reach sales without useful source information.",
    tone: "warning",
  },
] as const;
...
const AUDIT_DELIVERABLES = [
  {
    ...
    description: "The point in the property lead journey where tracking or source information stops being reliable.",
  },
  {
    icon: Gauge,
    title: "What it affects",
    description: "How the issue changes what you see in ad reports, analytics or the enquiries and viewing requests your sales team receives.",
  },
  {
    icon: Route,
...
const HEALTH_DIMENSIONS = [
  { icon: Code2, number: "01", title: "Did we capture the action?", description: "We check whether property enquiry forms, WhatsApp actions, booked viewings and other key lead actions are actually being recorded." },
  { icon: Route, number: "02", title: "Do we know which campaign it came from?", description: "We check whether the enquiry or viewing request can still be tied back to the ad, campaign or source that generated it." },
  { icon: Send, number: "03", title: "Does the source reach sales?", description: "We check whether useful source information survives into your sales inbox, lead system or CRM." },
] as const;

const PROCESS_STEPS = [
  { number: "01", title: "Apply", description: "Tell us how you generate property enquiries and which journey matters most." },
  ...
  { number: "03", title: "Trace the journey", description: "We review one path from campaign click through the property page to the sales destination." },
...
const AUDIT_FAQS: FAQItem[] = [
  {
    ...
    answer: "The page is designed for property developers, estate agencies, brokerages, property firms and other real estate businesses using digital acquisition to generate enquiries, viewing requests or sales opportunities.",
  },
  {
  ...
  },
  {
    ...
    answer: "One real estate business or brand, one website, one core property lead journey and up to two paid ad platforms where relevant. You receive a Tracking Health Scorecard with findings, business impact, priority and recommended next steps.",
  },
  {
    question: "Does this audit cover our whole CRM or sales process?",
    answer: "No. The free audit checks the source and conversion handoff only where it affects the scoped property lead journey. A full CRM, sales-process, call-tracking, offline-conversion or automation audit is separate.",
  },
  {
    question: "Can the audit verify closed-deal attribution?",
    answer: "We can assess whether the scoped journey preserves the campaign and conversion data needed to support downstream attribution into your sales or CRM handoff. We do not promise full closed-deal or revenue attribution where offline sales data and downstream systems are outside the free audit scope.",
  },
  {
...
const TRACKING_AUDIT_ANCHOR_CTA = {
  ...
  to: "/offer/tracking-audit/real-estate#claim",
} as const;
...
const TrackingAuditRealEstate = () => {
  ...
  const {
    ...
    defaultValues: { adPlatforms: [], marketingOptIn: false, industry: "real_estate" },
  });
  ...
  return (
    <>
      <SEO
        title="Free Conversion Tracking Audit for Real Estate | AlphaTrack Digital"
        description="Request a free conversion tracking audit for one real estate lead journey and see where campaign attribution, enquiry tracking or sales lead-source visibility may be breaking."
          ...
          <div className="mx-auto grid max-w-6xl gap-9 lg:my-auto lg:w-full lg:-translate-y-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(420px,480px)] lg:items-center lg:gap-14">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="pt-2 lg:pt-0">
              ...
              <p className="mt-5 max-w-[35rem] text-base leading-7 text-foreground/72 md:text-lg md:leading-8">
                We check what happens from the ad click to the property page, enquiry or viewing request, so you can see where real estate leads are being missed, misreported or losing their source before sales follow-up.
              </p>

              <div className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm text-foreground/78">
                {["No passwords", "One property lead journey", "Clear next steps"].map((item) => (
                  ...
                  <div className="mb-5">
                    ...
                    <h2 className="mt-4 text-xl font-semibold">
                      {step === 1
                        ...
                        : step === 2
                          ? "How do you generate property enquiries?"
                    ...
                    <p className="mt-1 text-[13px] leading-5 text-muted-foreground">
                      {step === 1
                        ...
                        : step === 2
                          ...
                          : "Tell us what you want to understand before you scale the next property campaign."}
                    </p>
                  </div>
                  ...
                  <form id="tracking-audit-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate aria-label="Request a Free Tracking Audit">
                    {step === 1 ? (
                      ...
                      >
                        ...
                        <Field label="Company / Organisation" htmlFor="f-company" error={errors.company?.message}>
                          <Input id="f-company" placeholder="Company or organisation name" autoComplete="organization" className={fieldClassName} aria-invalid={!!errors.company} aria-describedby={errors.company ? "f-company-err" : undefined} {...register("company")} />
                        </Field>
                      ...
                      >
                        <div className="space-y-3.5">
                          <Field label="How clear are you on where property enquiries come from?" htmlFor="f-maturity" error={errors.trackingMaturity?.message}>
                            <Controller control={control} name="trackingMaturity" render={({ field }) => (
                              <FormSelect
                                id="f-maturity"
                                label="How clear are you on where property enquiries come from?"
                                value={field.value}
                                onValueChange={field.onChange}
                          ...
                          <Field label="What property action matters most?" htmlFor="f-conversion" error={errors.primaryConversionType?.message}>
                            <Controller control={control} name="primaryConversionType" render={({ field }) => (
                              <FormSelect
                                id="f-conversion"
                                label="What property action matters most?"
                                value={field.value}
                                onValueChange={field.onChange}
                                options={CONVERSION_OPTIONS}
                                placeholder="Select main property action"
          ...
          <div className="relative grid items-center gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:gap-12">
            <div>
              ...
              <p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground">
                Your sales team gets the enquiry. What can disappear is the context that explains which ad, campaign, listing or channel brought that prospective buyer, tenant or investor to you.
              </p>
            </div>
        ...
        <div className="tracking-audit-scope-card mx-auto mb-10 max-w-6xl overflow-hidden rounded-[24px] border border-white/[0.07] bg-[linear-gradient(135deg,rgba(51,204,153,0.055),rgba(0,175,239,0.025)_52%,rgba(255,255,255,0.015))] md:mb-12">
          <div className="grid md:grid-cols-[1.05fr_1.95fr]">
            <div className="border-b border-white/[0.07] p-5 md:border-b-0 md:border-r md:p-7">
              ...
              <h2 className="mt-2.5 text-xl font-bold tracking-tight md:text-2xl">One real estate brand. One website. One property lead journey.</h2>
            </div>
            <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4 md:p-5">
              {[
                ["01", "One real estate business / brand"],
                ["02", "One website / domain"],
                ["03", "One core property lead journey"],
          ...
          >
            ...
            <div className="grid grid-cols-2 border-t border-white/[0.07] bg-primary/[0.035] text-center">
              <div className="px-3 py-2.5">
                ...
                <p className="mt-1 text-[13px] font-semibold">Fix the viewing / CRM handoff</p>
              </div>
          ...
          <div className="container relative mx-auto px-5 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-5xl text-center">
              <h2 className="mx-auto max-w-5xl px-2 text-balance text-3xl font-bold leading-[1.12] tracking-tight sm:text-4xl md:text-[2.8rem] lg:text-5xl">
                <span className="block">Know which property campaigns are</span>
              ...
              <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-muted-foreground md:text-base">
                If you’re spending on property marketing but can’t confidently connect campaigns to enquiries, booked viewings or downstream opportunities, request a free audit.
              </p>
