...
const Index = () => {
  ...
  const [heroSwitchOn, setHeroSwitchOn] = useState(false);

  useEffect(() => {
    if (shouldReduceMotion) return;

    const intervalId = window.setInterval(() => {
      setHeroSwitchOn((prev) => !prev);
    }, 2400);
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-white/10 bg-[#111217] pb-20 pt-24 md:pt-28">
        <div className="pointer-events-none absolute inset-0">
          <div
            className="absolute inset-0"
            style={{
              background: [
                "radial-gradient(ellipse 62% 45% at 50% 0%, rgba(255,255,255,0.05) 0%, transparent 68%)",
                "radial-gradient(ellipse 38% 34% at 18% 72%, rgba(0,175,239,0.08) 0%, transparent 70%)",
                "radial-gradient(ellipse 35% 32% at 84% 70%, rgba(51,204,153,0.06) 0%, transparent 70%)",
              ].join(", "),
            }}
          />
        ...
        <div className="container relative mx-auto px-4 lg:px-8">
          ...
          >
            <span className="inline-flex items-center rounded-xl border border-white/10 bg-white/[0.06] px-5 py-2 text-sm font-semibold text-white">
              ATD Funnel Rescue Stack
            </span>

            <h1 className="mt-6 text-4xl font-semibold leading-tight text-white sm:text-5xl md:text-6xl">
              Why lose revenue to errors
              <span className="block">you can control?</span>
            </h1>

            <p className="mx-auto mt-4 max-w-2xl text-base text-slate-300 md:text-lg">
              Every failed handoff across ads, analytics, and CRM costs pipeline.
              <span className="block">ATD pinpoints the break and fixes it fast.</span>
            </p>

            <div className="mt-8 flex items-center justify-center">
              <button
                type="button"
                onClick={() => setHeroSwitchOn((prev) => !prev)}
                className={cn(
                  "group relative inline-flex h-[86px] w-[250px] items-center rounded-full border px-3 transition-all duration-500",
                  heroSwitchOn
                    ? "border-primary/80 bg-gradient-to-r from-primary to-secondary shadow-[0_10px_45px_rgba(0,175,239,0.35)]"
                    : "border-[#2563eb] bg-[#2563eb] shadow-[0_10px_45px_rgba(37,99,235,0.28)]"
                )}
                aria-pressed={heroSwitchOn}
                aria-label={heroSwitchOn ? "Switch back to Fix It" : "Switch to Fixed"}
              >
                <motion.span
                  className="absolute left-3 top-1/2 block h-[62px] w-[62px] rounded-full bg-white"
                  animate={heroSwitchOn ? { x: 166, y: "-50%" } : { x: 0, y: "-50%" }}
                  transition={{ duration: shouldReduceMotion ? 0 : 0.45, ease: "easeInOut" }}
                />
                <span className="w-full text-center text-[40px] font-bold leading-none text-white">
                  {heroSwitchOn ? "FIXED" : "FIX IT"}
                </span>
              </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 text-left sm:grid-cols-4">
              {heroMetrics.map((metric) => (
                <div key={metric.label} className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3">
            ...
            <div className="sr-only" aria-hidden="true">
              {heroMetrics.map((metric) => (
                <div key={`duplicate-${metric.label}`}>
          ...
          <div className="relative mx-auto mt-12 hidden h-[350px] max-w-6xl lg:block" aria-hidden="true">
            {[{ title: 'No Saved Events', body: 'Browser + server events are not syncing in Meta.', x: 'left-[5%]', y: 'top-[9%]', delay: 0 },
              { title: 'OTP Error', body: 'Lead verification flow drops users before submit.', x: 'right-[4%]', y: 'top-[9%]', delay: 0.6 },
              { title: 'CVV Error', body: 'Checkout validation blocks qualified traffic.', x: 'left-[42%]', y: 'bottom-[10%]', delay: 1.1 },
              { title: 'Redirect Timeout', body: 'Users are not returning to confirmation page.', x: 'right-[24%]', y: 'bottom-[4%]', delay: 1.4 }].map((card) => (
              <motion.div
                key={card.title}
                className={`absolute ${card.x} ${card.y} w-64 rounded-xl border border-rose-100/70 bg-white text-left shadow-[0_18px_48px_rgba(0,0,0,0.35)]`}
                animate={shouldReduceMotion ? {} : { y: [0, -10, 0] }}
                transition={{ duration: 3.8, repeat: Infinity, delay: card.delay, ease: 'easeInOut' }}
              >
                <div className="rounded-t-xl bg-rose-100 px-4 py-2 text-sm font-semibold text-rose-500">{card.title}</div>
                <p className="px-4 pb-4 pt-3 text-sm text-slate-700">{card.body}</p>
              </motion.div>
            ))}
          </div>

          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <Button asChild size="lg" className="gap-1.5 rounded-full px-8">
              <Link to="/book-a-call">
                Book a Call <ArrowUpRight className="h-4 w-4" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="rounded-full border-white/25 bg-white/5 px-8 text-white hover:bg-white/10">
